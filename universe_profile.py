...
def validate_profile(raw):
    ...
    if focus is not None and focus not in all_symbols:
...
def apply_profile(config, profile):
    ...
    # OMNI consumers import these immutable projections, not ALL_TICKERS.
    # A profile is an explicit instrumented pilot; legacy editorial members
    # and activation must never leak from the operator's default configuration.
    config.SIGNAL_ELIGIBLE_TICKERS = tuple(plan["active_tickers"])
    config.SIGNAL_ELIGIBLE_TICKER_SET = frozenset(plan["active_tickers"])
    config.EDITORIAL_ONLY_TICKERS = ()
    config.LEGACY_EDITORIAL_ONLY_TICKERS = ()
    config.EDITORIAL_ONLY_TICKER_SET = frozenset()
    config.EDITORIAL_COVERAGE_TICKERS = config.SIGNAL_ELIGIBLE_TICKERS
    config.EDITORIAL_COVERAGE_TICKER_SET = config.SIGNAL_ELIGIBLE_TICKER_SET
    config.EDITORIAL_COVERAGE_MODE = "off"
    config.CORE_TICKER_ALIASES = MappingProxyType({
        entry["symbol"]: tuple(entry["aliases"]) for entry in profile["equities"]})
    config.EDITORIAL_TICKER_ALIASES = MappingProxyType({})
    config.GDELT_TICKER_ALIASES = config.CORE_TICKER_ALIASES
    config.ISSUER_REGISTRY = MappingProxyType({})
    config.UNIVERSE_PROFILE = profile
    config.SCHEMA_VERSION = "2.9"


def profile_policy_manifest(profile):
    """Reproducible profile policy, independent of process-global configuration.

    Source permission rules are unchanged. Explicit profile metadata permits
    another process to validate membership without loading private configuration.
    """
    from coverage_policy import SOURCE_POLICIES, _fingerprint
    profile = validate_profile(profile)
    plan = profile_plan(profile)
    tickers = plan["active_tickers"]
    equities = [entry["symbol"] for entry in profile["equities"]]
    issuer_like = [t for t in tickers if not t.startswith("^") and not t.endswith("=F")]
    manifest = {
        "cohort_version": "profile-v1", "registry_version": "profile-assertions-1",
        "source_policy_version": "source-purpose-1", "mapping_version": "profile-aliases-1",
        "mode": "off", "activation": "manual", "cloud_model_budget_usd": 0,
        "license_attestation": False, "registry_fingerprint": fingerprint(profile),
        "source_policy_fingerprint": _fingerprint(SOURCE_POLICIES),
        "unapproved_uses": ["raw_storage", "model_processing", "commercial_redistribution", "excerpt_redistribution"],
        "collector_targets": {
            "prices_technicals": tickers, "sec_cik_lookup": tickers,
            "options": equities, "earnings": equities, "social_attention": issuer_like,
            "fmp_symbols": issuer_like, "apewisdom_all_stocks_pages": 5,
            "apewisdom_4chan_pages": 1, "fmp_primary_requests": 1,
            "fmp_max_requests_with_entitlement_fallback": 2,
            "gdelt_query_policy": "profile-aliases-three-batches",
        },
    }
