...
function renderAll() {
    ...
    const renderers = [renderQuality, renderPrices, renderTechAlerts, renderSectorRotation,
        ...
        renderCEO, renderWhispers, renderTwitter, renderInsiders, renderEditorialFocus,
        renderEventTimeline, renderMarketAndSocialContext];
...
function renderQuality() {
    ...
    const expectedTickers = Array.isArray(DATA.universe?.instrumented_tickers)
        ? DATA.universe.instrumented_tickers
        : DATA.universe?.tickers;
    const expected = Array.isArray(expectedTickers) ? expectedTickers.length : 'Unknown';
    kv(summary, 'Viewing', SNAPSHOT_LABEL);
    kv(summary, 'Numeric price coverage', `${prices.filter(p => finite(p.price)).length} / ${expected} instrumented members`);
...
function renderEditorialFocus() {
    const container = reset('focus-card');
    const focus = object(section('deep_dive'));
    if (!Object.keys(focus).length) { empty(container, 'No editorial-focus record supplied.'); return; }
    kv(container, 'Status', focus.status || 'Unknown');
    kv(container, 'Ticker', focus.ticker || 'None selected');
    kv(container, 'Selection mode', focus.selection_mode || 'Unknown');
    kv(container, 'Headline', focus.headline || 'Unavailable');
    kv(container, 'What changed', object(focus.what_changed).text || 'Unavailable');
    kv(container, 'Why it matters', object(focus.why_it_matters).text || 'Unavailable');
    kv(container, 'Evidence grade', focus.evidence_grade || 'Unknown');
    kv(container, 'Contradictions assessed', text(focus.contradictions_assessed));
    container.append(node('div', 'record-meta', `${meta(focus)}. Editorial materiality is not an expected-return estimate.`));
    for (const evidence of rows(focus.evidence).slice(0, 20)) {
        const item = node('div', 'record-meta', `${text(evidence.publisher || evidence.provider)} · ${text(evidence.title)}`);
        appendExternalLink(item, evidence.canonical_url || evidence.link, ' Open evidence');
        container.append(item);
    }
}
function timelineRecord(family, record) {
    const title = record.title || record.headline || record.description || record.change_type || record.event_type || record.ticker || 'Record';
    const date = record.target_date || record.earnings_date || record.event_date || record.date || record.as_of;
    const item = node('div', 'text-item');
    appendTag(item, 'source-tag', family);
    item.append(node('div', '', title), node('div', 'record-meta', `Ticker ${text(record.ticker)} · Date ${text(date)} · Status ${text(record.status || record.change_type || record.event_type)}`));
    appendExternalLink(item, record.canonical_url || record.link || record.url, 'Open source');
    return item;
}
function renderEventTimeline() {
    const container = reset('event-timeline');
    const families = [
        ['Biotech news', 'biotech_news'], ['FDA catalysts', 'fda_catalysts'],
        ['FDA advisory meetings', 'fda_advisory_meetings'], ['Registry changes', 'registry_changes'],
        ['Earnings calendar', 'earnings_calendar'],
    ];
    let total = 0;
    for (const [label, key] of families) {
        const records = rows(section(key)); total += records.length;
        records.slice(0, 100).forEach(record => container.append(timelineRecord(label, record)));
    }
    if (!total) empty(container, 'No dated event records in these families. Check source health before interpreting absence.');
}
function renderMarketAndSocialContext() {
    const regime = reset('regime-card');
    const value = object(section('regime'));
    if (Object.keys(value).length) {
        for (const [label, measurement] of Object.entries(value)) kv(regime, label, text(measurement));
    } else empty(regime, 'No regime measurements supplied.');
    const container = reset('social-context-list');
    const families = [
        ['Social attention', 'social_attention'], ['Social threshold records', 'social_alerts'],
        ['Baseline threshold records', 'baseline_alerts'], ['Unanchored social context', 'social_whispers_unanchored'],
    ];
    let total = 0;
    for (const [label, key] of families) {
        const records = rows(section(key)); total += records.length;
        records.slice(0, 100).forEach(record => {
            const item = node('div', 'text-item');
            appendTag(item, 'source-tag', label);
            item.append(node('div', '', record.title || record.text || record.tag || record.signal || record.ticker || 'Record'),
                        node('div', 'record-meta', `${meta(record)} · Ticker ${text(record.ticker)} · Mentions ${text(record.mentions)}`));
            appendExternalLink(item, record.canonical_url || record.link || record.url, 'Open source');
            container.append(item);
        });
    }
    if (!total) empty(container, 'No market/social context records supplied.');
}
