...
## OMNI-compatible integration

Default launches retain schema 2.8 and the existing shadow editorial/evidence
intake policies. Profile launches use schema 2.9, embed their normalized profile,
and update every instrumented consumer before imports. They force editorial mode
off and inherit no legacy editorial names. A missing/null `focus_ticker` requests
dynamic evidence-gated focus; an explicit member is still subject to evidence
eligibility. Configuration changes require a fresh Python process and workspace.

`python marketbot.py doctor --universe profiles/us-core.example.json --workspace workspaces/core`
performs read-only diagnostics without provider requests or probe-file creation.
It reports dependency availability, contact syntax, credential presence, workspace
identity and saved-brief health counts. File permissions are only an estimate.
