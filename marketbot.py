...
def run_profile(profile, workspace, command="run", ledger_command=None):
    ...
    """
    captured = ("coverage_policy", "export_for_gemini", "signals", "ledger.config")
    if any(name in sys.modules for name in captured) or any(name.startswith("agents.") for name in sys.modules):
        raise RuntimeError("Profile execution requires a fresh process; configuration consumers are already imported")
    import config
    if command == "run":
...
def main(argv=None):
    ...
    try:
        if args.command == "doctor":
            ...
            profile = load_profile(args.universe) if args.universe else None
            print(json.dumps(diagnose(ROOT, args.workspace, profile), indent=2, allow_nan=False))
        elif args.command in ("inspect", "diff"):
