...
class BriefToolsTests(unittest.TestCase):
    ...
    def test_editorial_members_are_not_missing_price_targets(self):
        self.brief["universe"]["tickers"].append("EDITORIAL")
        self.brief["universe"]["instrumented_tickers"] = ["AAA", "BBB"]
        coverage = inspect_brief(self.brief)["coverage"]
        self.assertEqual(coverage["expected_universe"], 2)
        self.assertEqual(coverage["missing_numeric_prices"], ["BBB"])

    def test_dynamic_focus_changes_do_not_change_configuration_identity(self):
        newer = copy.deepcopy(self.brief)
        newer["universe"]["focus_ticker"] = "BBB"
        self.assertTrue(diff_briefs(self.brief, newer)["configuration_comparable"])
    ...
    def test_real_schema_fixture_uses_sections(self):
        ...
        self.assertEqual(result["new_headline_records"], fixture["sections"]["headlines"])
